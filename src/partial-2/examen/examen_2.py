...
def excel(conteo_formas):
    """Guardar el conteo de formas en excel."""
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Conteo de formas"

    total_formas = sum(conteo_formas.values())
    ws.append(["Total de Formas", total_formas])

    ws.append(["Forma", "Cantidad"])
    for forma, cantidad in conteo_formas.items():
        ws.append([forma, cantidad])

    excel_path = os.path.join(public_path, "reporte_excel.xlsx")
    wb.save(excel_path)
...
def reporte(conteo_formas):
    ...
    # Enviar el reporte con el total de formas a excel
    send_to_excel = tk.Button(
        window, text="Guardar en Excel", command=lambda: excel(conteo_formas))
    send_to_excel.pack()
